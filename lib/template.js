...
module.exports = {
  ...
  */
  result_list: function (filelist) {
    ...
    let resultLang = ["C++", "C"];
    while (i < filelist.length) {
      var sresult = resultVal[parseInt(filelist[i].result)+1];
      var slang = resultLang[parseInt(filelist[i].solve_lang)];
      list = list + `
      <tr>
        <td><h5 class="ui center aligned header">${filelist[i].solve_id}</h5></td>
        <td><h5 class="ui center aligned header"><a href="/problem/${filelist[i].solve_problem}">${filelist[i].solve_problem}</a></h5></td>
        <td>${sresult}</td>
        <td>${filelist[i].solve_sec}</td>
        <td>${slang}</td>
        <td>${filelist[i].solve_len}</td>
        <td>${filelist[i].solve_sec}</td>
      </tr>
      `
      // `<li style="padding : 3px; border-bottom : 1px solid;">제출번호 : ${filelist[i].solve_id} // 닉네임 : ${result.nickname} // <a href="/problem/${filelist[i].solve_problem}">Problem ID : ${filelist[i].solve_problem}</a> // 결과 : ${sresult} // 코드길이 : ${filelist[i].solve_len}</li>`;
