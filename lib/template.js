...
module.exports = {
  ...
  topbar: function (request, response, select="") {
    let sideMenu = [
      ...
      {name : "Log out", link :"/auth/logout"}
    ];
    if (!auth.isOwner(request, response)) {
      sideMenu = [
        {name : "Join", link :"/auth/join"},
  ...
  problem_list: function (filelist, request, response) {
    ...
    while (i < filelist.length) {
      ...
      if (auth.isOwner(request, response)) {
        // id부여를 제대로 구현 안해놨어요..
        var que = `SELECT result FROM Solve WHERE solve_member=${request.session.passport.id} AND solve_problem=${filelist[i].pb_id}`;
...
>>>>>>> c7e7ec0f7cedbe07bc4e236d2c3bfa05e72503f3
}
